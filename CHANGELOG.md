...
- 防呆与资产校验继续收紧：`doc_sync_check.py` 为 `已生效` / `已落地` 文档增加“最近更新”真实日期证据链（`YYYY-MM-DD` 占位即失败），`已落地` 必须引用至少一个 `docs/` 之外的代码路径；新增交付物目录版本布局校验（`requirements` / `design` / `tasks` / `upgrade` / `api` / `sql` 根级只允许 `README.md` 与模板，实际文件必须在 `vX.Y.Z/` 下）；存量 29 篇文档的“最近更新”占位按 git 最后提交日期补齐。`check_all.py` 新增 skill 库资产完整性校验（每个 skill 目录必须有 `SKILL.md` 与 `agents/openai.yaml`，frontmatter `name` 与目录一致、`description` 非空，`_template` 除外），并新增 `--scan-docs` 透传 doc-sync `--scan-all` 供 CI 定期存量治理。
