...
对已有行为、共享组件、配置或脚本，先保留工作区状态并建立可重复的改前行为基线；改后执行同一基线。与改动一起新增的测试不能单独证明旧行为。
...
- 先读 `AGENTS.md`
- 先读 `docs/project-profile.md`，确认技术栈、验证命令和专项 skill 是否适用
...
3. 修改既有行为时使用 `safe-code-change` 建立兼容基线
4. 若信息还不够，先理解现状
5. 再最小改动实现或进入设计流程
6. 再 findings-first review
7. 最后聚焦人工检查点
