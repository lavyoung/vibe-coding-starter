...
只写一份裸 `AGENTS.md`（3~5 条“请先写文档”的约定）5 分钟就能建好，但“先写文档”只是口头约定：没有状态闸门、没有同步矩阵、没有机器校验，草稿照样会被当成实现依据，换会话照样失忆。

本模板把这 5 分钟延长到约 30 分钟，换回的是：

- **规则可执行**：`已接受 / 已生效 / 已落地` 状态闸门 + `.doc-sync.json` + CI 强制校验，改代码忘改文档会被检查拦下
- **换会话不失忆**：单点快照、演进索引、交接模板让新会话只靠仓库文档恢复上下文
- **AI 行为可复用**：17 个 skill（含 `AGENTS.md` 0.3 强制门禁），不用每次重新教

需要提醒的是这套体系有持续成本：每个版本要同步维护需求 / 设计 / 升级 / 契约文档；**一次性脚本或临时试验项目直接用裸 `AGENTS.md` 更合适**。完整对比与代价说明见 [README.md](README.md) 的“为什么值得用”。
...
```bash
python scripts/doc_sync_check.py --changed-file src/routes/tasks.js --changed-file docs/api/v1.1.0/tasks.yaml
bash scripts/doc_sync_check.sh --changed-file src/routes/tasks.js --changed-file docs/api/v1.1.0/tasks.yaml
```

```powershell
./scripts/doc_sync_check.ps1 --changed-file src/routes/tasks.js --changed-file docs/api/v1.1.0/tasks.yaml
```
