...
- 优先最小改动，不做顺手重构。
- 先匹配当前代码风格，再补能力。
- 改动前先查当前模块、相邻模块和已有脚本里是否已经存在可复用实现，优先扩展现有代码，不要重复造轮子。
- 新增公共抽象、通用组件、工具方法前，先说明现有实现为什么不能承载；若只是单点需求，优先留在当前边界内解决。
- 代码以“让下一个接手的人一眼看懂”为目标，优先可读性、命名清晰和职责单一，避免为未来假设提前堆复杂度。
- 若需求不明确，先显式说明假设。
- 不要把“待讨论方案”直接写进实现。
